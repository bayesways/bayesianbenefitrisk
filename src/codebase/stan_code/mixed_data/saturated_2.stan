...
transformed data {
  ...
  matrix[K, K] L_R = diag_matrix(rep_vector(1, K));
}
...
transformed parameters{
  ...
  if (cov_num==1){
    for (g in 1:cov_num){
    L_inv[g,] = mdivide_left_tri_low(diag_matrix(sigma[g,]), I);
    }
    for (n in 1:N){
      resid = L_inv[1,] * (yc[n] - alpha_c[grp[n],]);
      z[n,] = transpose(alpha_b[grp[n],] + tail(L_R * append_row(resid, zb[n]), Kb));
    }
  }
  else if (cov_num>1){
    for (g in 1:cov_num){
...
model{
  ...
  if (cov_num==1){
    for (n in 1:N){
      yc[n,] ~ multi_normal_cholesky(alpha_c[grp[n],], diag_pre_multiply(sigma[1, ], L_R[1:Kc, 1:Kc]));
    }
  }
  else if (cov_num>1){
    for (n in 1:N){
      yc[n,] ~ multi_normal_cholesky(alpha_c[grp[n],], diag_pre_multiply(sigma[grp[n], ], L_R[1:Kc, 1:Kc]));
    }
  }
...
generated quantities{
  ...
  for (g in 1:number_of_groups){
    if (cov_num==1){
      ...
      Marg_cov[g, ] = multiply_lower_tri_self_transpose(diag_pre_multiply(full_sigma[1,],L_R)); 
    }
    else if (cov_num>1){
      full_sigma[g,] = append_row(sigma[g,], rep_vector(1, Kb));
      Marg_cov[g, ] = multiply_lower_tri_self_transpose(diag_pre_multiply(full_sigma[g,],L_R)); 
    }
